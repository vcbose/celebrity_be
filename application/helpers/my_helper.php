...
{  
	...
	if(!empty($keys)){
		...
		$CI->db->select('*');
		$CI->db->from('cb_setting');
		$CI->db->where('setting_type' , $keys);
		$query = $CI->db->get();
...
function settings($a_where = array()) 
{

	$CI = get_instance();
	$CI->load->model('setting/setting_model');
	$setting= $CI->setting_model->get_setting('', $a_where);
	
	$result = []; 
	foreach ($setting as $key => $value) {
		$result[$value->setting_type][$value->setting_id] = array($value->setting_name => $value->setting_value);
	}
	return $result;
}
...
function getCBResponse( $response_code ){
	...
	switch ($response_code) {
		...
		case 'ER_AUTH_SUS':
			...
			break;
		case 'ER_AUTH_ACC':
			$msg = 'Your account is suspended and is not permitted to access this feature';
			break;
		case 'ER_LIMIT':
			$msg = 'Rate limit exceeded';
			break;
		case 'ER_CHAT_IN':
			$msg = 'You cannot send messages to users who are not following you.';
			break;
		case 'ER_CHAT_OUT':
			$msg = 'There was an error sending your message: reason';
			break;
		case 'ER_INTR_IN_ALRDY':
			$msg = 'You`ve already requested to follow user.';
			break;
		case 'ER_REQUEST':
			$msg = 'This request looks like it might be automated. To protect our users from spam and other malicious activity, we can’t complete this action right now.';
			break;
		case 'SUC_INTR_IN':
			$msg = 'You interest has been sent!';
			break;
		case 'ER_INTRW_IN':
			$msg = 'Could not prcoceed!';
			break;
		case 'SUC_INTRW_IN':
			$msg = 'Interview has been successfully scheduled!';
			break;
		default:
